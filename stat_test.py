...
import numpy as np
import pandas as pd
from scipy.stats import false_discovery_control, shapiro, ttest_rel, wilcoxon
...
def stat_test_hyperparameter(
    df, hyperparameter, hp_value1, hp_value2, alternative
):
    return pd.Series(
        [
            *shapiro_test(df, hyperparameter, hp_value1, hp_value2),
            *ttest_rel_test(
                df,
                hyperparameter,
                hp_value1,
...
def wilcoxon_test(df, hyperparameter, hp_value1, hp_value2, alternative):
    return wilcoxon(
        np.concatenate(
            df[df[hyperparameter] == hp_value1]["rmse_list"].values
        ),
        np.concatenate(
            df[df[hyperparameter] == hp_value2]["rmse_list"].values
        ),
        alternative=alternative,
        axis=None,
    )
...
def adjust_p_value(
    res_df, cols, group_attributes, method_func=false_discovery_control
):
    for col in cols:
        res_df[f"adj_{col}"] = res_df.groupby(by=group_attributes)[
            col
        ].transform(lambda x: method_func(x))
    return res_df
...
def statistical_significance_test(data_stat_tests):
    ...
    for hyperparameter in hyperparam_list:
        if hyperparameter == "n_hidden":
            ...
            res_dfs_n_hidden = []
            for n_hidden1, n_hidden2 in itt.combinations(
                hyperparameter_values[hyperparameter], 2
            ):
                res_df_partial = (
                    dmm_stat_tests.groupby(by="context")
                    .apply(
                        lambda df,
                        hp1=n_hidden1,
                        hp2=n_hidden2: stat_test_hyperparameter(
                            df,
            ...
            res_df_n_hidden["adj_t-test_p-value"] = res_df_n_hidden.groupby(
                by=["context"]
            )["t-test_p-value"].transform(lambda x: false_discovery_control(x))
            ...
            res_df_partial = (
                dmm_stat_tests.groupby(by=["context", "n_hidden"])
                .apply(
                    lambda df, hp=hyperparameter: stat_test_hyperparameter(
                        df,
                        hyperparameter=hp,
                        hp_value1="L1",
                        hp_value2="L2",
            ...
            for pair in pairs:
                ...
                res_df_partial = (
                    ...
                    .apply(
                        ...
                        hp=hyperparameter,
                        v1=hp_value1,
                        v2=hp_value2: stat_test_hyperparameter(
                            df,
                            hyperparameter=hp,
                            hp_value1=v1,
                            hp_value2=v2,
                ...
                res_df_partial[
                    "test_kind"
                ] = f"RMSE_{hp_value1} > RMSE_{hp_value2}"
    ...
    res_df["-log10_adj_Wilcoxon_p-value"] = -np.log10(
        res_df["adj_Wilcoxon_p-value"]
    )
