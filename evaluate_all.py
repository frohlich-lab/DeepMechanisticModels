...
for dataset in ["train", "test"]:
    ...
    pretraining = pd.read_csv(
        outdir / f"{SAMPLES}_evaluate_pretrain_cross_sample_{dataset}.csv", index_col=0
    )
    # training
    ...
    # average
    avg = pd.read_csv(outdir / f"{SAMPLES}_evaluate_average_{dataset}.csv", index_col=0)
    avgs[dataset] = np.power(10, avg.rmse.apply(np.log10).mean())

    # per sample
    df_ps = pd.read_csv(
        outdir / f"{SAMPLES}_evaluate_pretrain_per_sample_{dataset}.csv", index_col=0
    )
