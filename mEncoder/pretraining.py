...
def generate_per_sample_pretraining_problems(
    importer: PetabImporterPysb, model: str, dataset: str, sample: str
) -> PetabImporterPysb:
    """
...
def generate_cross_sample_pretraining_problem(ae: MechanisticAutoEncoder) -> Problem:
    """
    ...
    problem = Problem(
        ...
        x_names=x_names,
        lb=[
            parameter_boundaries_scales[xname.split("_")[-1]][0] - 2.0
            for xname in x_names
        ],
        ub=[
            parameter_boundaries_scales[xname.split("_")[-1]][1] + 2.0
            for xname in x_names
        ],
    )
...
def store_and_plot_pretraining(
    result: Result, outdir: Path, prefix: str, plot_waterfall: bool = True
):
    """
