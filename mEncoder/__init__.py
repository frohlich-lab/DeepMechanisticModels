...
def apply_objective_settings(problem, pathway_name):
    ...
    elif isinstance(problem.objective, pypesto.objective.jax.JaxObjective):
...
COLLECTED_ESTIMATION_OUTFILE_TEMP = (
    "__".join(["{context}", "{samples}", "{n_hidden}", "{alpha}", "full"]) + ".hdf5"
)

ESTIMATION_OUTFILE_TEMP = (
    "__".join(["{context}", "{samples}", "{n_hidden}", "{alpha}", "{job}"]) + ".hdf5"
)
