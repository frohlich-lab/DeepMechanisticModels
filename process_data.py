...
from mEncoder import load_pathway, data_dir, pretrain_dir, PER_SAMPLE_OUTFILE_TEMP
...
def training_samples(wildcards):
    ...
    return (
        samples[: int(np.round(n_samples / n_splits * split))]
...
def test_samples(wildcards):
    ...
    n_samples = len(samples)
    return samples[
        int(np.round(n_samples / n_splits * split)) : int(
            np.round(n_samples / n_splits * (split + 1))
        ) :
    ]
...
if __name__ == "__main__":
    ...
    if DATA == "dream_cytof":
        ...
        measurement_table_phospho.drop(columns=["treatment", "fileID"], inplace=True)
        ...
        measurement_table = pd.concat(
            [measurement_table_phospho, measurement_table_proteomics]
        )
    ...
    measurement_table[petab.OBSERVABLE_ID] = measurement_table[
        ...
        lambda x: observable_id_to_model_expr(x, observable_mode, model) + "_obs"
